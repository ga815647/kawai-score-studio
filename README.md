...
- 日式兒童教材風格的靠左圓角標題區。
- 無框彩色數字為主，五線譜為輔；不使用音符框、底色或延長底線。
- 彩色數字為 26px，中文歌詞為 18px；中文比前版醒目，簡譜比前版收斂。
- 中文底部到五線譜第一條線的允許距離為 12–16px。
- 上點與下點直接位於彩色數字正上方／正下方。
...
- 中文歌詞逐起音對位。
- 產生器使用 VexFlow 音頭左右邊界的幾何中心作為共同 X 座標，彩色數字與中文不再使用偏左的 tick X。
- 簡譜列高度、歌詞位置、五線譜靠近量、字級及實際允許距離都由 `scorebook.yaml` 定義。

## 五線譜與對齊

- 五線譜由固定版本的 VexFlow 5.0.0 產生 SVG，不再使用手工繪製的音頭、音桿或譜號。
- VexFlow 依每首歌的調性與拍號產生譜號、調號、拍號、附點、八分音符連桿及複合時值連結線。
- 產生器在 VexFlow 完成格式化後，以 `getNoteHeadBeginX()` 與 `getNoteHeadEndX()` 的中點定位每個 event。
